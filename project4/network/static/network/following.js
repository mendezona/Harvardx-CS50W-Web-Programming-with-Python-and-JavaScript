...
function following(pageNumber) {
    ...
    postsDisplayArea.innerHTML = '<h5>Posts from users you are following</h5>';
    ...
    fetch(`${pageNumber}`, {
        ...
        if (posts["0"]["pageNumber"] > 1) {
            ...
            previousButton.addEventListener('click', function() {
                following(posts["0"]["pageNumber"] - 1);
            });
        }
        ...
        for (i = 5; i < posts.length; i++) {
            ...
            fetch(`http://127.0.0.1:8000/like/${posts[i]["id"]}`)
                .then(response => response.json())
                .then(likes => {
                    ...
                    likeButton.addEventListener('click', function() {
                        ...
                        setTimeout(function() {
                            fetch(`http://127.0.0.1:8000/like/${postIDLikeButton}`)
                                .then(response => response.json())
                                .then(likes => {
                                    console.log(likes);
                                    likeCount.innerHTML = `	&#10084; ${likes["likeCount"]}` + "<br/>";
...
function followUser(username, followingStatus) {
    let csrftoken = getCookie('csrftoken');
    fetch(`http://127.0.0.1:8000/followers/${username}`, {
        method: 'PUT',
        headers: {
            "X-CSRFToken": csrftoken
        },
...
function userProfile(username, pageNumber) {
    ...
    fetch(`http://127.0.0.1:8000/followers/${username}`)
        ...
        .then(followers => {
            ...
            fetch(`http://127.0.0.1:8000/user/${username}/${pageNumber}`)
            .then(response => response.json())
            .then(posts => {
            // Print posts received
            console.log(posts);
